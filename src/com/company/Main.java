...
public class Main {
    ...
    public static int[] heroesHealth = {300, 280, 250, 200, 1000, 290, 250};
    public static int[] heroesDamage = {20, 15, 25, 20, 10, 30, 25};
    public static String[] heroesAttackType = {"Physical", "Magical", "Kinetic", "Lucky", "Golem", "Thor", "Berserk"};
    ...
    public static void bossHits() {
        ...
        } else if (bossDefenceType == heroesAttackType[6] && (heroesHealth[6] > 0)) {
            int berserkBlock = (int) (Math.random() * (bossDamage - 2) + 1);
            heroesHealth[6] = heroesHealth[6] - (bossDamage - berserkBlock) +bossDamage;
            block = berserkBlock;
            for (int i = 0; i < heroesHealth.length; i++) {
                if (heroesHealth[i] > 0) {
                    if (heroesHealth[i] < bossDamage) {
    ...
    public static void heroesHit() {
        if (bossDefenceType == heroesAttackType[6] && (heroesHealth[6] > 0)) {
            bossHealth = bossHealth - (heroesDamage[6] + block) + heroesDamage[6]; // Далее отнимем лишний урон [6]
